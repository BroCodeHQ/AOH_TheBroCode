...
@app.post("/custom_story")
async def create_story_json(input_text: InputText):
    """
    Creates a short story with provided senario using OpenAI API.

    Args:
        text: Based on the children's imagination

    Returns:
        Video Output of the story
    """

    prompt = f"""
- Craft a short narrative suitable for children aged 2 to 5 years old.
- The child's imagination is : "{input_text}"
- The story would be created based on the imagination of a child. The child may think of any incident picturing themselves as the superhero or any other situation with details.
- The story should be appropriate for a child and not generate NSFW content
- The story should be interesting and relatable for the child based on their personal incident taken as input""" + prompt_full
...
<html>
    <head>
        <title>Customized Story Re-generation</title>
    </head>
    <body>
        <div class="bg-gray-200 p-4 rounded-lg shadow-lg">
            <h1>Customized Story Re-generation FastAPI@{__version__}</h1>
            <ul>
